...
impl<'a> Solver<'a> {
    ...
    fn bfs(&mut self, node: Index) -> bool {
        ...
        while let Some(node) = queue.pop_front() {
            for (from, to, (capacity, _flow)) in self.graph.neighbors(node) {
    ...
    fn get_edges(&self, n: usize, m: usize) -> Vec<(Index, Index, (i32, i32))> {
        let mut edges = Vec::new();
        for i in 0..n {
            for j in 0..m {
                if let Some(edge) = self.graph.edges.get(&i).unwrap().get(&(j)) {
                    if edge.1 > 0 {
                        edges.push((i, j, *edge));
                    }
                }
...
fn main() {
    ...
    let t = solver.solve();
    let mut edges = solver.get_edges(n, n);
